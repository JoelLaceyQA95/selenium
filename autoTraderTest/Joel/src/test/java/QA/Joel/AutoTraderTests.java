...
public class AutoTraderTests {
	...
	private NewCarSearch newCarSearch;
    ...
    public void signUp() throws IOException {
        ...
        signInElements.userPassword(userPass);
...
//    		}
	...
	@Test
	public void newCarSearch() throws IOException{
		ExtentTest test = report.createTest("search new car test");
        test.log(Status.INFO, "Fourth Test is Starting ");
        signInElements = PageFactory.initElements(webDriver, SignInElements.class);
        webDriver.navigate().to(url1);
        newCarSearch = PageFactory.initElements(webDriver, NewCarSearch.class);
        try {
			Thread.sleep(1000);
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
        ...
        newCarSearch.newCarSearch();
        String userPostcode = spreadSheetRow.get(2);
        newCarSearch.postcode(userPostcode);
        try {
			Thread.sleep(1000);
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
        ...
        String endMsg = newCarSearch.breadcrumbs();
        String expectedMsg = "Search Results";
        		
        try {
        	assertEquals(expectedMsg, endMsg);
        	ScreenShot.take(webDriver, "newCarSearch");
        	test.pass("successful new car search");
        }catch(AssertionError ae) {
        	ScreenShot.take(webDriver, "newCarSearchFail");
        	test.fail("new car search failed");
        	throw ae;
        }
	}
